...
function test() {
  ...
  const comparisonHtml = readDoc('comparison.html');
  ...
  expectIncludes(indexHtml, 'href="./comparison.html"', 'landing page should link to the comparison page');
  ...
  expectIncludes(catalogHtml, 'href="./comparison.html"', 'catalog page should link to the comparison page');
  expectIncludes(comparisonHtml, 'How Nexus Prime differs from other multi-agent control planes and frameworks', 'comparison page should render the hero title');
  expectIncludes(comparisonHtml, './assets/competitive-landscape.json', 'comparison page should load the checked-in competitive landscape asset');
  expectIncludes(comparisonHtml, 'rel="canonical"', 'comparison page should declare a canonical URL');
  expectIncludes(comparisonHtml, '50+ GitHub stars', 'comparison page should explain the star threshold');
  expectIncludes(comparisonHtml, 'Tier 1: direct control planes / developer orchestrators', 'comparison page should clearly separate tier 1');
  expectIncludes(comparisonHtml, 'Tier 2: broader multi-agent frameworks / platforms', 'comparison page should clearly separate tier 2');
  ...
  expectIncludes(knowledgeBaseHtml, 'href="./comparison.html"', 'knowledge base should link to comparison');
  ...
  expectIncludes(architectureHtml, 'href="./comparison.html"', 'architecture page should link to comparison');
  ...
  expectIncludes(readme, 'https://sir-ad.github.io/nexus-prime/comparison.html', 'README should point to the comparison page');
  ...
  expectIncludes(readme, '## Competitive Landscape', 'README should include the comparison teaser section');
  ...
  expectIncludes(comparisonHtml, 'Menu', 'comparison page should expose a mobile navigation control');
