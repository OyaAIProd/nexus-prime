# Nexus Prime — Full Operating Protocol

This document contains the complete operating protocol for Nexus Prime.
For the concise agent-facing instructions, see [CLAUDE.md](../CLAUDE.md).
...
nexus_recall_memory(query="<task in 10 words>", k=8) when you need the raw matches directly
nexus_memory_stats()
...
Each worker writes `.agent/runtime/context.json` and `.agent/runtime/context.md`. The instruction gateway writes `.agent/runtime/packet.json` and `.agent/runtime/packet.md`. Use the packet as the machine-facing brief. Treat AGENTS.md as the human operator manual.
...
```
nexus_session_bootstrap    nexus_orchestrate        nexus_recall_memory
nexus_memory_stats         nexus_store_memory       nexus_plan_execution
nexus_optimize_tokens      nexus_mindkit_check      nexus_ghost_pass
nexus_spawn_workers        nexus_session_dna        nexus_list_skills
nexus_list_workflows       nexus_list_hooks         nexus_list_automations
nexus_list_specialists     nexus_list_crews         nexus_federation_status
```
